...
async function snapshot() {
  let { version } = await config(),
    json = (
      await exec(
        `gh release view v${version} --json assets -R phyrondev/phyron-skia-canvas`,
      )
    ).stdout,
...
async function ensureRelease(version) {
  let tag = `v${version}`;
  try {
    await exec(`gh release view ${tag} -R phyrondev/phyron-skia-canvas`);
  } catch {
    console.log(`Release ${tag} not found, creating draft...`);
    await exec(
      `gh release create ${tag} --draft --title ${tag} --generate-notes -R phyrondev/phyron-skia-canvas`,
    );
  }
}
...
async function upload() {
  ...
  try {
    ...
    await exec(
      `gh release upload v${version} ${artifact} --clobber -R phyrondev/phyron-skia-canvas`,
    );
  } catch (e) {
...
function compile(...args) {
  let optimization =
    ...
    customFeatures =
      ...
      (args[args.indexOf("custom") + 1] || "").replace(/[^[a-z0-9\_\-\,]/g, ""),
  ...
  if (!isSrcRepo)
    throw Error(
      `Cannot compile from npm version of skia-canvas: clone source from ${REPO_URL}`,
    );
