...
class ImageData {
  constructor(...args) {
    ...
    if (!["srgb"].includes(colorSpace)) {
      // TODO: add display-p3 when supported…
...
function pixelSize(colorType) {
  const bpp = ["Alpha8", "Gray8", "R8UNorm"].includes(colorType)
    ? 1
    : ["A16Float", "A16UNorm", "ARGB4444", "R8G8UNorm", "RGB565"].includes(
          colorType,
        )
      ? 2
      : [
        ...
        ? 4
        : ["R16G16B16A16UNorm", "RGBAF16", "RGBAF16Norm"].includes(colorType)
          ? 8
          : colorType == "RGBAF32"
            ? 16
            : 0;
