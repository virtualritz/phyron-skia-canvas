...
class Path2D extends RustClass {
  ...
  interpolate(path, weight) {
    return Path2D.interpolate(this, ...arguments);
  }

  // boolean operations
  complement(path) {
    return Path2D.op("complement", this, ...arguments);
  }
  difference(path) {
    return Path2D.op("difference", this, ...arguments);
  }
  intersect(path) {
    return Path2D.op("intersect", this, ...arguments);
  }
  union(path) {
    return Path2D.op("union", this, ...arguments);
  }
  xor(path) {
    return Path2D.op("xor", this, ...arguments);
  }
  ...
  simplify(rule) {
    return Path2D.effect("simplify", this, ...arguments);
  }
  unwind() {
    return Path2D.effect("unwind", this);
  }
  round(radius) {
    return Path2D.effect("round", this, ...arguments);
  }
  offset(dx, dy) {
    return Path2D.effect("offset", this, ...arguments);
  }
