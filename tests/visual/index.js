...
function renderTest(canvas, name, option, format) {
  ...
  return new Promise((res, rej) => {
    let cb = async (err) =>
      err ? rej(err) : res(await canvas.toBuffer(format));

    if (option.gpu != undefined) canvas.gpu = option.gpu;
...
app.get("/", async (c) => {
  ...
  if (Object.keys(query).length == 0) {
    ...
    opts.gpu =
      query.gpu && query.gpu != "null" ? !!parseInt(query.gpu) : defaults.gpu;
    ...
    if (query.alpha != null && opts.cc.length == 7 && opts.cc[0] == "#") {
      ...
      const a = Math.max(
        Math.min(Math.round(255 * parseFloat(query.alpha)), 255),
        0,
      );
      if (Number.isFinite(a)) opts.cc += a.toString(16).padStart(2, "0");
    }
...
for (let i = 2; i < process.argv.length; ++i) {
  ...
  if (i == 2 && parseInt(arg)) port = parseInt(arg);
    ...
    defaults.width = parseInt(process.argv[++i]) || defaults.width;
  else if (/^-?h/i.test(arg))
    defaults.height = parseInt(process.argv[++i]) || defaults.height;
  else if (/^-?c/i.test(arg)) defaults.cc = process.argv[++i];
  else if (/^-?b/i.test(arg)) {
    defaults.bc = process.argv[++i];
    option.bc_default = false;
