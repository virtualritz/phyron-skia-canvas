...
function drawPinwheel(
  ctx,
  { scl = 0.5, offset = 0, len = 100, cb = null } = {},
) {
  ...
  for (var i = 0; i <= 12; i++) {
    ...
    if (cb) cb(cos, sin, -sin, cos);
    else ctx.transform(cos, sin, -sin, cos, 0, 0);
  }
}
...
tests["transform() with DOMMatrix"] = function (ctx) {
  let cb = isWeb
    ? null
    : (a, b, c, d) => ctx.transform(new DOMMatrix(a, b, c, d, 0, 0));
  drawPinwheel(ctx, { scl: 0.5, offset: 0, len: 100, cb });
};
...
tests["transform() with matrix-like object"] = function (ctx) {
  let cb = isWeb
    ? null
    : (a, b, c, d) => ctx.transform({ a: a, b: b, c: c, d: d, e: 0, f: 0 });
  drawPinwheel(ctx, { scl: 0.5, offset: 0, len: 100, cb });
};
...
tests["generic font family fallback sans-serif"] = function (ctx) {
  ...
  ctx.font =
    '24px None_suchFont, sans-serif, "Times New Roman", "Times", monospace';
  ctx.textAlign = "center";
...
function drawShadowPattern(
  ...
  if (rectCb) rectCb(20, 20, 100, 100);
  else ctx[rectFn](20, 20, 100, 100);
  ...
  if (color2) ctx.shadowColor = color2;
...
function drawScaledBoxes(
  ctx,
  { sz = 200, sp = 50, x = sp, y = sp, scl = 0.25 } = {},
) {
  ctx.fillStyle = "black";
  ctx.shadowColor = "#c00";
  for (let i = 1; i <= 9; ++i) {
...
tests["drawImage() SVG natural size with scaling from drawImage"] = function (
  ctx,
  done,
) {
  var img = new Image();
...
tests["drawImage() SVG natural size with scaling from ctx"] = function (
  ctx,
  done,
) {
  var img = new Image();
...
tests["SVG no natural size drawImage(img,sx,sy,sw,sh,x,y,w,h)"] = function (
  ctx,
  done,
) {
  var img = new Image();
...
tests["SVG with natural size drawImage(img,sx,sy,sw,sh,x,y,w,h)"] = function (
  ctx,
  done,
) {
  var img = new Image();
...
tests["SVG with natural size drawImage(img,sx,sy,sw,sh,x,y,w,h) with scaling"] =
  function (ctx, done) {
    var img = new Image();
    img.onload = function () {
